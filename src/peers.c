...
void *_co_build_func()
{
    ...
    assert(gbuf);
    assert(gbuflen);
    return gfunc(gctx, gppw, gbuf, gbuflen);
}
#endif
...
int _build_port(char *buf) { return (uint16_t)(buf[0] << 8 | buf[1]); }
...
int _read_message(char *buf, int buflen, _peer_context *ctx)
{
    ...
    case MSG_PIECE: {
        int n = message_parse_piece(msg, msglen, &(ctx->state->buf), ctx->state->buflen,
                                    ctx->state->index);
        if (n == 0)
        {
            return 0;
    ...
    while (state->downloaded < pw->length)
    {
        if (!ctx->choked)
        {
            while (state->requested < pw->length)
            {
                ...
                int recvslen;
                recvslen =
                    _peer_send_request(ctx->sock, pw->index, state->requested, block_size, &recvs);
                if (recvslen == 0)
                {
                    goto ERROR_RETURN;
                }
...
#ifdef USE_CO
        gfunc = _peer_download;
        gctx = ctx;
        gppw = ppw;
        gbuf = recv2;
        gbuflen = unchoke_recvslen + interested_recvslen;
        main_co = co_new(NULL, 0);
        void *func = _co_build_func();
        new_co = co_new(func, 1024 * 1024);
        co_resume(new_co);
#else
