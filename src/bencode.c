...
void bencode_init(struct bencode *ctx, const void *buf, size_t len)
{
    ctx->tok = 0;
    ctx->toklen = 0;
    ctx->buf = buf;
...
void bencode_free(struct bencode *ctx)
{
    free(ctx->stack);
    ctx->stack = 0;
}

static int bencode_get(struct bencode *ctx)
{
...
static int bencode_peek(struct bencode *ctx)
{
    if (!ctx->buflen)
        return -1;
...
static size_t bencode_push(struct bencode *ctx)
{
    if (ctx->size == ctx->cap)
    {
        void *newstack;
...
static int bencode_integer(struct bencode *ctx)
{
    int c;
...
static int bencode_string(struct bencode *ctx)
{
    int c;
...
int bencode_next(struct bencode *ctx)
{
    int c, r;
    size_t i;
    void **keyptr = 0;
